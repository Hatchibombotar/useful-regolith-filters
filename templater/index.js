...
glob("BP/templates/**/*.json", null, function (err, files) {
    ...
    for (template of currentTemplates) {
        glob(`${template.description.use_on}**/*.json`, null, function (er, files) {
            for (file of files) {
                const requiredTemplates = JSON.parse(fs.readFileSync(file)).use_templates
                if (requiredTemplates == undefined) continue
                for (templateUsed of requiredTemplates) {
